...
3.  **Gemini 分析**:
    - `review-analysis-prompt.md` テンプレートを使用。
...
- `gemini` CLI のラッパー。
- `command -v` による事前チェック、`envsubst` によるプロンプト生成、`gh` CLI によるコメント投稿を担当。

### C. Prompt Template (`.github/workflows/context/review-analysis-prompt.md`)
- Review Analyst としてのロール定義。
...
- **Error Handling:** `gemini` コマンド不在時や必須環境変数欠落時に、適切なエラーログを出力して終了すること（無限ループや誤爆を防ぐ）。
