...
# issue-kit のバージョン (タグまたはコミット SHA)
ISSUE_KIT_REF="${ISSUE_KIT_REF:-@main}"

# 相対パス（./）を明示して呼び出す
CONTEXT_FILE=$(uv tool run --from "git+https://github.com/masa-codehub/issue_creator_kit.git${ISSUE_KIT_REF}" ick dispatch --labels "$PR_LABELS" --config-path "$TEMP_CONFIG" --get-context || echo "UNKNOWN")
