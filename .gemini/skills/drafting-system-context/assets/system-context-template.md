...
- ガードレール: 「誰が」「何を把握し」「どうすることで」「どんな価値（数値的目標含む）」を得るかを定義してください。
- 記述例: 「工場管理者が重機ごとの『サイクル電力』をリアルタイムに把握し、非効率な稼働パターンを特定することで、年間電力コストの15%削減を支援する。」
-->
...
- ガードレール: 内部に保持すべき「核（競争力）」と、外部に任せる「詳細」を明確に区切ってください。
- 記述例（境界）: 「【サイクル解析】 工程の区切りを推論するロジックは競争力の核であるため内部に保持。」
-->
...
- 自システム（System）、アクター（Person）、外部システム（System_Ext）を明確に区別してください。
-->
...
- ガードレール: 「Aを優先するためにBを許容する」という意思決定を明文化してください。
- 記述例: 「[鮮度 vs 正確性] 請求根拠となるため正確性を最優先。ネットワーク遅延による10秒程度の表示遅延は許容し、重複排除を保証する。」
-->
...
- ガードレール: 実装者が守るべき根本的な方針（不変データ、ドメイン純粋性など）を定義してください。
-->
