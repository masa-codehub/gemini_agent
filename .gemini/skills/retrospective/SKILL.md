...
- **W (わかったこと):**
  ...
  - **アウトプット例 (Markdown):**
    ...
    ```
- **T (次やること / 仮説立案):**
  - `analyzing-intent` スキルの「多角的仮説立案」に準じ、「Y」および「W」で収集した事実を根拠に3つの仮説（実証的、飛躍的、逆説的）を立てる。
...
- **T (Try): 具体的な改善策と資産化 (多角的仮説立案)**
  KとPの分析結果から、`analyzing-intent` スキルの「多角的仮説立案」に準じ、**「なぜうまくいったのか/いかなかったのか」**についての3つの仮説を立て、仕組みとして定着させるためのアクションを定義する。
